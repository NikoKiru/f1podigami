...
**Fix (#206, extended #<PR>): wait in-run rather than retry across runs.** `src/wait_for_results.py` polls every 3 min for up to **5 h** until the pending round appears — `/{season}/last/results.json` for a race, the last row of `/{season}/qualifying.json` for qualifying. Notes:
