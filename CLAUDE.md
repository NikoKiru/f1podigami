...
```
src/fetch/    → API calls to the Jolpica F1 API (api.jolpi.ca/ergast/f1), plus OpenF1 for the newest round only → data/*.json
...
- A cheap **`check` job** runs `src/check_update_due.py` **every 15 min** (`cron: 2,17,32,47 * * * *`; offset off the hour to dodge GitHub's top-of-hour load, which often drops scheduled runs; no network, no secret) and proceeds only when a pending session is newer than the data: a race newer than `podigami.json`'s `asOf`, or the next race's qualifying not yet covered by `postQuali`. The guard **arms 3 h before each session's scheduled start** (`ARM_BEFORE`), so a run is already waiting when results appear. A weekly run (`0 7 * * 1`) forces an unconditional `--full` reconciliation; `workflow_dispatch` takes `mode` (auto/full), `force`, and `wait` (hold the runner like a scheduled run — successor runs set it).
...
**Fix (#206, extended #323): wait in-run rather than retry across runs.** `src/wait_for_results.py` polls every 3 min for up to **5 h** until the pending round appears — `/{season}/last/results.json` for a race, the last row of `/{season}/qualifying.json` for qualifying. Notes:
- It polls **aggregate** feeds on purpose — the round-indexed endpoints lag for hours (see #178 below).
- The guard arms **3 h before** each race and qualifying session (`check_update_due.ARM_BEFORE`): a run is already waiting when the data lands in **93% of races / 91% of qualifying sessions**, replaying 14 days of the post-2026-08-27 scheduler — measured against **OpenF1's** publish time (`docs/superpowers/specs/2026-09-10-openf1-fast-lane-design.md` section 4), assuming OpenF1 has the result ~40 min after the flag. The fast lane now does use OpenF1, and at the 2026 Spanish GP it came later than that (about three hours after the start), so a run that armed right at the start of the 3 h window can time out (start + 2 h) before OpenF1 has the result; the successor hand-over then carries the watch on, exactly as it does for Jolpica. Against Jolpica's own, later and more variable publish, the hand-over chain below keeps a watch alive across the gap, so coverage is at least as high. Arming early costs only idle runner time.
- A timeout is **not** a failure: the watcher reports `published=false` and the job **hands over to a successor run immediately, skipping the pipeline** (`gh workflow run update.yml -f mode=auto -f wait=true`, built-in token, `actions: write` scoped to just the `update` job) while the session is < 12 h past its scheduled start (`SUCCESSOR_WINDOW`) — so a later link in the chain runs the pipeline once the round actually shows up, instead of two pipelines racing Jolpica's hourly rate limit minutes apart. Only after that window elapses does the run fall through and run the pipeline anyway (idempotent), as a fallback. A run whose watcher finds nothing pending at all reports `published=none`, which also skips the pipeline — there being nothing to do, running `update.py` again right after the hold would just repeat a no-op.
- The `update` job's first step waits (≤ 15 min) for any open `auto/update-data` PR to merge, so a successor never computes on a `main` that is about to change; the pipeline step then re-syncs to `origin/main` first, so a PR that merges during a long watch is included.
...
- The `update` job carries `timeout-minutes: 345` (15 min PR wait + 5 h watcher + ~20 min pipeline, under GitHub's 6 h cap); the pipeline step itself (`Fetch latest data and rebuild site`) additionally carries `timeout-minutes: ${{ needs.check.outputs.mode == 'full' && 60 || 25 }}` — full reconciliations take ~24 min and never run the watcher, so they get 60; a normal run after a 5 h watch keeps 25 so the job stays under 345 min — so a runaway fetch fails loudly instead of the whole job being cancelled silently at 345 min.
- Trigger on demand: `gh workflow run update.yml -f mode=auto -f force=true` (or `-f mode=full`; add `-f wait=true` to hold the runner and poll). **On a race weekend, always add `-f wait=true`** — a plain dispatch that lands the pending slot acts immediately and skips the watcher, ending the hand-over chain (no successor is dispatched from it), and a dispatch without `wait=true` still queues behind the `update-data` concurrency group, which can hold for the chain's ~5¾ h — a dispatch waiting in that queue can itself be replaced by a successor before it ever runs (`cancel-in-progress: false` never cancels a run that has already started).
- The `update-data` concurrency group now holds for hours during a chain, not the usual ~20 min: the `watchdog` job (part of the same workflow run as the held `update` job) only gets a fresh report once per link, so during a chain its cadence degrades from ~15 min to however far apart the links land; and a successor dispatched while a pending weekly `--full` run is still queued behind the group cancels that queued run — accepted, since `--full` reconciles again the following Monday.

#### OpenF1 fast lane (#327)

`src/fetch/fetch_openf1.py` runs right after the Jolpica fetchers. For the newest race and qualifying session Jolpica does not have yet, it writes OpenF1's classification (api.openf1.org, available from ~30 min after the session ends) into `podiums.json` / `race_results.json` / `qualifying.json` in Jolpica's exact row shape. One race has been measured live so far, and from a **local** fetcher run polling every 3 min rather than the production pipeline: at the 2026 Spanish GP (R14, started 13:00 UTC) OpenF1's `session_result` was still a 404 at 15:40 UTC, the fast lane first filled the round at 16:01 UTC with the stewards' check clear — about three hours after the start — and Jolpica published at about 19:45 UTC, nearly four hours later, with rows identical to OpenF1's (the podium, the race header and all 22 result rows, grid positions included). The Monza rehearsal replayed data days later with `--now`, so it proves the mapping, not the timing. Replaying 2026 R1–R13 from frozen fixtures (`tests/test_fetch_openf1.py`), every race the stewards' check lets through produces podium and race-result rows identical to Jolpica's. Qualifying is compared **by position**: Jolpica's stored row order differs in R5 and R10, and OpenF1 omits a car that set no time (R4), so a qualifying confirmation can reorder rows or add a car — a harmless diff.
- **Fail closed.** No write on any OpenF1 error, an unmatched/cancelled session (matched by UTC date + 6 h window), an unknown car number, a surname mismatch, an unknown team name (`TEAM_TO_CONSTRUCTOR` — a rebrand needs a new alias), or a held **stewards' check** (`src/fetch/stewards_gate.py`: open incident, unserved drive-through or time penalties that would change the podium *trio*, OpenF1's positions 1–3 disagreeing with its own race times, or a top-three row with no driver number). Holds ~22% of races (backtest: 65/83 published); those keep the Jolpica timing.
- **Rate limit.** OpenF1's free tier caps a client at 30 requests/minute (HTTP 429 + `Retry-After: 60`). `fetch/openf1.py` spaces request starts ≥ 2.1 s apart and retries a 429 after its Retry-After, clamped to 0–65 s, at most twice per request and 130 s in total per process, then fails closed. Go through that client; never add a second one or extra sleeps. The fixture recorder (`tests/fixtures/openf1/record_fixtures.py`) lifts the total cap, aborts on any failed request instead of writing an empty entry, and writes deterministic gzip.
...
- **The confirmation hand-over needs a real fill.** `fetch_openf1.py` writes `filled=race|qualifying` to `GITHUB_OUTPUT`, and the pipeline step (`id: pipeline`) carries it. `Check whether Jolpica still has to confirm` runs only on `published == 'fast' && steps.pipeline.outputs.filled != ''`. A fast run whose fill failed closed doesn't hand over, so a same-round retry waits for the next scheduled run instead of repeating a pipeline minutes later.
- **The Jolpica watcher can't be taken down by the fast lane.** `wait_for_results.py` imports `fetch_openf1` lazily inside `_openf1_ready`, so an import error or exception there is logged and read as "not ready".
...
- **Accepted trade-off (confirmation hold).** While a round stays unconfirmed past its 12 h successor window, every due scheduled run holds the concurrency group for a 5 h confirmation watch until Jolpica confirms. The watchdog's cadence degrades for that stretch, and a session that arms meanwhile waits for the next run. `--fail-on-stale` still surfaces a round unconfirmed > 48 h after its session, at most one watch late.
- **Accepted trade-off (fast-lane retries).** While OpenF1 fills keep failing, a scheduled run that reaches a still-pending race ends right after its fast pipeline instead of holding for Jolpica, so Jolpica's result then waits for the next scheduled run.
...
- **Where to read the timings.** Each watch prints a UTC stamp when it ends, accurate to the 3-min poll: `OpenF1 has round … at …` in the fast run, then `Jolpica has round … at …` in the confirmation run. Use those two to measure the gain per race.
