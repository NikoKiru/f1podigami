...
- The guard arms **3 h before** each race and qualifying session (`check_update_due.ARM_BEFORE`): a run is already waiting when the data lands in **93% of races / 91% of qualifying sessions**, replaying 14 days of the post-2026-08-27 scheduler — measured against **OpenF1's** publish time (`docs/superpowers/specs/2026-09-10-openf1-fast-lane-design.md` section 4), which this PR does not use. Against Jolpica's own, later and more variable publish, the hand-over chain below keeps a watch alive across the gap, so coverage is at least as high. Arming early costs only idle runner time.
...
`src/fetch/fetch_openf1.py` runs right after the Jolpica fetchers. For the newest race and qualifying session Jolpica does not have yet, it writes OpenF1's classification (api.openf1.org, available from ~30 min after the session ends) into `podiums.json` / `race_results.json` / `qualifying.json` in Jolpica's exact row shape. OpenF1 usually has the classification within about an hour of the race, often hours before Jolpica: at the 2026 Spanish GP (R14, 2026-09-13), the fast lane filled the round by 16:01 UTC with the stewards' check clear, while Jolpica published only at about 19:45 UTC, roughly five hours after the flag, with rows identical to OpenF1's (the podium, the race header and all 22 result rows, grid positions included). For every 2026 round it lets through, its rows were identical to Jolpica's.
